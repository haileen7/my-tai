...
- Package registry metadata for versions. Before adding a dependency, run
  ...
  equivalent, then read the docs for that major version. For Composer packages:
  `composer show <pkg> --all`.
...
For h-dashboard (Laravel 13.33, Livewire 4.4, MaryUI, Tailwind 4.3, PHP 8.5,
Pest 4, PostgreSQL 16 + PostGIS, Redis):

- Read `AGENTS.md` first — it is the authoritative project instruction set, and
  its `references/*.md` files carry the detailed API/deployment/data-model docs.
- Use Laravel Boost MCP `search_docs` for version-specific Laravel docs and
  `application_info` before assuming any package API.
- Use Context7 MCP `query_docs` with libraryId `/laravel/docs` (or
  `/websites/laravel_13_x`) for framework questions.
- Use `codegraph explore` / `codegraph query` for code-structure questions
  before grep/read_file.
- Prefer Laravel/Boost docs over memory for Livewire 4 single-file components,
  MaryUI, Sanctum, and Spatie Permission behavior — this project pins unusual
  versions (Livewire 4.4, spatie/laravel-permission 8.3).
